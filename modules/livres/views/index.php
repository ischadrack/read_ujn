...
$niveaux_stmt = $db->prepare("SELECT DISTINCT niveau_classe FROM livres WHERE niveau_classe != '' ORDER BY niveau_classe");
$niveaux_stmt->execute();
$niveaux = $niveaux_stmt->fetchAll(PDO::FETCH_ASSOC);

// Récupérer les langues pour les filtres
$langues_stmt = $db->prepare("SELECT DISTINCT langue FROM livres ORDER BY langue");
$langues_stmt->execute();
$langues = $langues_stmt->fetchAll(PDO::FETCH_ASSOC);
?>
    ...
    <div class="lg:ml-64 min-h-screen">
        ...
        <div class="p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    <h1 class="text-3xl font-fredoka font-bold text-gray-900 dark:text-white">Catalogue des Livres</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-2">Gestion de la collection de livres de la bibliothèque</p>
                </div>
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="add" class="bg-library-600 hover:bg-library-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <i class="fas fa-book text-library-600 text-2xl"></i>
                        </div>
                        <div class="ml-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Empruntés</p>
                            <p class="text-2xl font-semibold text-gray-900 dark:text-white">
                                <?php echo $stats['empruntes']; ?>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8">
                ...
                <form method="GET" class="grid grid-cols-1 md:grid-cols-7 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" placeholder="Titre, auteur, code..."
                    ...
                    <div>
                        ...
                        <select name="langue" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-library-500 focus:border-transparent text-gray-900 dark:text-white">
                            <option value="">Toutes les langues</option>
                            <?php foreach ($langues as $langue): ?>
                            <option value="<?php echo htmlspecialchars($langue['langue']); ?>" <?php echo isset($_GET['langue']) && $_GET['langue'] == $langue['langue'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($langue['langue']); ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="disponible" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-library-500 focus:border-transparent text-gray-900 dark:text-white">
                            <option value="">Tous</option>
                            <option value="1" <?php echo isset($_GET['disponible']) && $_GET['disponible'] == '1' ? 'selected' : ''; ?>>Disponibles seulement</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="sort" class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-library-500 focus:border-transparent text-gray-900 dark:text-white">
                            ...
                            <option value="categorie_id" <?php echo $sort == 'categorie_id' ? 'selected' : ''; ?>>Catégorie</option>
                            <option value="quantite_disponible" <?php echo $sort == 'quantite_disponible' ? 'selected' : ''; ?>>Disponibilité</option>
                        </select>
                    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Disponibilité</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-16 w-12">
                                            ...
                                            <img src="../../../../assets/uploads/livres/<?php echo htmlspecialchars($livre['photo']); ?>" class="h-16 w-12 rounded object-cover" alt="Couverture">
                                            <?php else: ?>
                                            <div class="h-16 w-12 rounded bg-library-100 dark:bg-library-900 flex items-center justify-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style="background-color: <?php echo htmlspecialchars($livre['categorie_color']); ?>20; color: <?php echo htmlspecialchars($livre['categorie_color']); ?>;">
                                        <?php echo htmlspecialchars($livre['categorie_nom']); ?>
                                    </span>
                                    <?php if ($livre['niveau_classe']): ?>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                        <?php echo htmlspecialchars($livre['niveau_classe']); ?>
                                    </div>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">
                                        <?php echo $livre['quantite_disponible']; ?>/<?php echo $livre['quantite_stock']; ?>
                                    </div>
                                    <?php if ($livre['quantite_empruntee'] > 0): ?>
                                    <div class="text-xs text-orange-600 dark:text-orange-400">
                                        <?php echo $livre['quantite_empruntee']; ?> empruntés
                                    </div>
                                    <?php endif; ?>
                                    <?php if ($livre['reservations_actives'] > 0): ?>
                                    <div class="text-xs text-blue-600 dark:text-blue-400">
                                        <?php echo $livre['reservations_actives']; ?> réservations
                                    </div>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        <a href="view?id=<?php echo $livre['id']; ?>" class="text-library-600 hover:text-library-900 dark:text-library-400 dark:hover:text-library-300 transition-colors" title="Voir les détails">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="edit?id=<?php echo $livre['id']; ?>" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 transition-colors" title="Modifier">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <?php if ($livre['quantite_disponible'] > 0): ?>
                                        <a href="../../emprunts/views/add?livre_id=<?php echo $livre['id']; ?>" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 transition-colors" title="Nouvel emprunt">
                                            <i class="fas fa-hand-holding"></i>
                                        </a>
                                        <?php endif; ?>
                                        <button onclick="deleteLivre(<?php echo $livre['id']; ?>)" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors" title="Supprimer">
